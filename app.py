...
def is_valid_course_code(text):
    match = re.match(r"^[A-Z]{2,4}\s?\d{3}$", text.strip().upper())
    return bool(match)

def find_best_match(user_question, model, embeddings, df, threshold=0.5):
...
if user_input:
    ...
    # Course Code Direct Match
    elif is_valid_course_code(user_input.upper().strip()):
    ...
    # Main logic
        ...
        # Step 1: Try structured course match
        query_info = extract_course_query(normalized_input)

        # Deep follow-up using memory
        ...
        response = result if result else "😕 I couldn’t find an answer to that. Try rephrasing it?"
